...
5. **Depth（自顶向下从开始节点到当前SUnit节点的最长路径）**
   ...
   计算方法：遍历当前SUnit节点的前驱节点，对每个前驱节点的depth和latency求和，取max
...
该调度器作用于**寄存器分配后**，采用**自顶向下**的拓扑排序方式

着重考虑通过最长关键路径(Depth、Height)**提升指令重排后的流水线性能**

### 8.8.1 Post-RA-TDList 调度器实现

通过选项`post-RA-scheduler`来启用Post-RA-TDList调度器，该调度器由`Schedule-PostRATDList类`实现

调度器用AvailableQueue存储当前可调度的节点指令，用Pending存储时延超过阈值的可调度节点，用CurCycle表示当前阈值，用Sequence存储调度后的指令序列

### 8.8.2 Post-RA-TDList 调度器具体实现流程
...
使用`clang++ -mllvm -enable-misched --target=riscv32-unknown-elf -o main.o -c xxx.cpp -O2`

经过LLVM寄存器后端分配后生成的MIR指令如下：
...
1. 按照调度区间的粒度构建SUnit节点的依赖图
2. 对依赖图中的指令自顶向下进行调度
3. 比较AvailableQueue中元素优先级（Height越大越先调度），对于Depth大于阈值的则存入Pending
4. **当AvailableQueue中没有元素时，阈值CurCycle+1**，再判断Pending中Depth和更新后的CurCycle大小，是否放回AvailableQueue
...
上一个调度器使用了SUnit的Depth和Height属性，Post-RA-MISched调度器则增加了**更多的启发式因素**来计算AvailableQueue中可调度指令的优先级，停**滞周期、关键资源、必需资源**

* 停滞周期——指令访存所占用的时钟周期，反映了CPU因为执行某些指令需要等待的时钟周期
* 关键资源——描述指令访问**调度区间内**资源的时钟周期开销
* 必需资源——描述指令**跨调度区间**访问资源的时钟周期开销

`-enable-post-misched`启用该调度器，调度算法由`ScheduleDAGMI类`实现
...
循环调度（SMS）是针对循环体进行的指令调度

### 8.10.1 循环调度算法实现（包括调度所需的启发式属性）

SMS是针对循环实现的且与架构无关的**软流水指令调度框架**

SMS通过将不同迭代（每次循环称为一次迭代）的指令**同时发射**，来提升并行度

**对于循环中无直接依赖的指令**，可以放在同一时刻进行发射
...
2. **构建依赖图并计算相关属性**
   ...
   为循环体中基本块包含的指令构建依赖关系，**并增加跨迭代过程中的指令依赖**

3. **计算调度所需的启发式属性**

   1. **`ResMII`（资源最小启动间隔）—— 用来描述硬件资源限制下的循环基本块指令的最小间隔**

      编译器一般用**循环基本块中的指令数➗最繁忙的硬件资源的总数**来做简单的近似值计算，但LLVM使用**`DFA`（确定性有限自动机）**这一复杂的计算方法来模拟CPU使用硬件资源执行指令的过程来计算ResMII

      DFA计算过程如下：

      * 按照**基本块指令使用硬件关键资源**（比如ALU单元）的**数量由高到低排序**，也就是说使用关键资源多的指令优先执行
      * 按上一步骤的指令顺序调用DFA以保存关键资源
      * 如果DFA不够（指当前资源已经被使用，需要等待相应硬件资源执行完才会空出），则新增DFA
      * 迭代2，3步骤，直到所有的指令都有DFA来保存资源
      * 最终DFA的数量就是ResMII，即ResMII等于DFA的数量

   2. **`RecMII`（循环依赖最小启动间隔）—— 描述循环依赖集合的最小启动间隔**

      如果存在跨迭代的依赖，我们称之为依赖图中存在循环依赖

      计算如下：

      * 通过约翰逊电路算法，找到循环基本块中所有的循环依赖集合

      * 遍历循环依赖集合，计算每个循环依赖的执行间隔 **`II = ceil(delay / distance)`**

        distance表示**循环执行的迭代次数**，LLVM中默认为1；delay表示**循环依赖集合中指令执行的时钟周期**

        （就是时间➗次数）

      * 选择**最大的**循环依赖集合的II作为RecMII

   3. **`MII`（最小启动间隔）—— 描述每次循环迭代执行的最小启动间隔**

      `MII =  max（ResMII ， RecMII）`

   4. **`ASAP`（最早调度时间）—— 指令最早允许被调度的时间**

      如果节点u有前驱节点集合，用v去遍历每一个前驱节点：`u的ASAP = max(v的ASAP ➕ 节点执行所需的时钟周期 ➖ ∆ ✖️ MII)` [∆表示当节点u和节点v存在跨迭代依赖时，他两之间的依赖边时延]

      如果没有前驱节点，则ASAP = 0

   5. **`ALAP`（最迟调度时间）—— 指令最晚允许被调度的时间**

      同上

      `u的ASAP = max(v的ASAP ➖ 节点执行所需的时钟周期 ➕ ∆ ✖️ MII)`  [∆表示当节点u和节点v存在跨迭代依赖时，他两之间的依赖边时延]

      如果没有前驱节点，则ALAP = 0

   6. **`MOV` —— 允许被调度的时间区间**

      一般MOV越大说明该指令的调度窗口越大，越容易被调度
...
4. **节点排序**
   ...
   * **循环迭代执行周期尽可能小**
     ...
     要求我们能按照顺序对一条链路进行调度，如果指令的前驱和后继节点均被调度了，那么它很容易失去调度窗口

   * **单次迭代中保持寄存器活跃区间(MaxLive)最小**

     要求我们尽可能减少寄存器的生命周期，也就是节点尽可能紧跟着它的前驱节点调度

   因此，节点排序很有必要，**既需要考虑到链路执行的顺序又要考虑寄存器的生命周期**

   **排序步骤：**

   1. 将循环基本块的所有递归集合按照RecMII由高到低进行排序

   2. 按照步骤1中的顺序遍历递归集合，对集合中的指令按照启发式因素Depth、Height、MOV进行排序，存入列表

      如果节点出现在多个递归中，则由RecMII最高的递归处理一次即可，其他的递归不再处理该节点

   3. 处理没有出现在任何递归集合中的节点，按照步骤2的启发式因素进行排序，直到所有节点都被遍历并存入列表

5. **调度**

   根据上一步生成的指令序列依次处理每一条指令，**着重考虑将当前节点尽可能靠近已经被调度过的前驱节点或后继节点**，减少寄存器压力

   **算法流程如下：**

   1. 将执行间隔II 设置为 MII ，初始化二维数组存放的调度结果（横坐标为指令被调度的时钟周期，纵坐标为在该时钟周期中被调度的指令序列）

   2. **如果当前节点 `u` 没有任何前驱或后继节点被调度过:**

      无约束——这种情况下，节点 `u` 的调度窗口较为宽松

      其调度区间为 `[Early_Start, Early_Start + II - 1]`，其中 `Early_Start` 等于该节点的ASAP（最早调度时间）

   3. **如果当前节点 `u` 只有相邻的前驱节点被调度过：**

      约束——节点 `u` 的开始时间必须晚于其所有前驱节点的完成时间。

      计算 `Early_Start = max(e_i)`，其中 `e_i` 是根据前驱节点 `v_i` 的调度时刻 `t_i` 和依赖时延以及前驱节点时延计算出的最早完成时刻。

      其调度区间为 `[Early_Start, Early_Start + II - 1]`

   4. **如果当前节点 `u` 只有相邻的后继节点被调度过：**

      约束——节点 `u` 的完成时间必须早于其所有后继节点的开始时间

      计算 `Late_Start = min(l_i)`，其中 `l_i` 是根据后继节点 `v_i` 的调度时刻 `t_i` 和依赖时延以及后继节点时延计算出的最晚开始时刻。

      其调度区间为 `[Late_Start - II + 1, Late_Start]`

   5. **如果当前节点 `u` 的前驱节点和后继节点均被调度过：**

      约束——此时，节点 `u` 的调度窗口被两端夹紧

      其调度区间为 `[Late_Start，Early_Start + II - 1]`

   6. 迭代与失败处理——如果上述过程节点找不到合适的调度周期，则将II + 1，重新开始从步骤1执行，直到所有节点都找到合适的调度区间

6. **生成并行化循环**

   根据调度结果重新构造循环指令，主要生成3个基本块：

   * Prologue：充当新的循环结构中的PreHeader基本块
   * Kernel：充当新的循环结构中的循环体基本块
   * Epilogue：充当新的循环结构中的循环退出基本块

   **LLVM根据每条指令被调度后所属的执行单元来决定哪些指令需要被放入Prologue、Kernel、Epilogue**

   每条指令的执行单元计算：

   `stage = floor(cycle / II)`，II为第五步成功调度的执行间隔

   最后要重命名迭代间依赖的指令操作数并更新各个基本块的控制流指令分支
...
寄存器分配前的调度算法会优先考虑重排的指令序列对寄存器分配压力的影响

寄存器分配后的调度算法则会专注于指令的流水性能

参考论文《A Comparision of List Scheduling Heuristics in LLVM Targeting POWER8》
