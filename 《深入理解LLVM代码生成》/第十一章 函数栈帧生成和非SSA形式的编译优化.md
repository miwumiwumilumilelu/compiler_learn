...
LLVM中目前关于冷热代码分离的两个相关实现即BasicBlockSection和MachineFunctionSplitter，二者都依赖Profile信息

1. `BasicBlockSection`依赖**PMU（性能监控单元）**信息，通过收集到的信息进行变化，再将变化后的信息作为编译输入，从而将基本块按照冷热进行分离

   虽然该功能已经合入LLVM主仓代码，但使用该功能**需要额外的Profile信息（一般是通过perf工具抓取原始信息后再进行处理）**，以前谷歌维护了开源的工具，但是目前该工具已经被谷歌删除

2. `MachineFuctionSplitter`（MFS）依赖Profile信息，根据基本块对应的Profile信息判断基本块是否冷热，按照三个类型分区：热、冷、异常

   按照执行频率区分冷热，基本块布局时，热基本块先放置，冷基本块后放置，并设置跳转指令在基本块末端，保障代码执行的正确性

   在以上基础上，MFS将多个函数的热代码放置在一起，冷代码放置在一起

   为了区别冷热代码，通常将热代码放在text.hot分区（可执行与可链接格式即ELF中的分区）；将冷代码放在text.unlikely分区。在链接时再进行处理



函数代码冷热分离功能实现如下：

1. 通过编译优化Pass识别基本块的冷热信息，并**设置基本块的冷热标记**
2. 根据冷热标记对基本块进行冷热分区划分，然后**为一些基本块添加跳转指令**（除了跳转指令外并不会添加函数栈帧等信息），并保留所有的执行路径
3. 在**机器码生成时根据冷热标记生成对应的机器码**（生成的机器码会统一放在热代码的最后）



值得注意的是，LLVM**中端优化有一个过程间优化HCS**（Hot-Cold Splitting，热-冷代码分离），作用与本节的**基于机器码的代码分离**MFS类似，但有以下区别：

1. HCS能够处理的函数必须是满足SESE（Single-Entry-Single-Exit单入口-单出口）的代码片段，每一个SESE区域独立处理；

   **MFS则是跨函数进行布局**；

2. HCS会将分离的冷代码生成一个函数，并考虑函数调用时的参数和返回值传递；

   **MFS并不会将冷代码提取为独立函数**，而是将多个函数的冷代码集中存放在一起，并添加跳转指令，实现较为简单；

3. HCS位于中端优化中，其他的优化（内联、CFG简化等）会影响IR，所以HCS和其他的优化需要考虑联动效果，而且一般要找到一个最为合适的Pass顺序是非常难的；

   **MFS是代码生成中优化的最后一步**，优化机会大，**且不需要和其他优化Pass协同（仅依赖于Profile信息**）；



由于HCS将冷代码生成一个新函数，所以会较MFS产生更多的代码量，而**新函数不仅要构造函数栈帧，还需要进行读取参数、执行冷代码、处理返回值等一系列操作**



### 11.4.4 代码布局优化比较

代码布局相关优化方法主要有：局部优化、PGO（基于Profile的优化，Profile Guided Optimization）、LTO（链接时优化，Link Time Optimization）、PLO（链接后优化，Post Link Optimization）

编译——汇编——链接——加载——运行

在编译阶段叠加PGO优化，在链接阶段进行LTO，在链接后针对二进制文件可以使用PLO，在运行时还可以使用JIT动态优化

1. 局部优化：主要指的是LLVM中端或者后端**针对单个函数进行优化**，比如MBP根据执行频率调整基本块顺序（执行频率可以来自PGO，也可以来自静态预测数据）、提取公共代码
2. PGO：通常有两种实现，1. 对源代码进行插桩（插桩代码主要收集分支执行情况），然后编译、运行，可以得到分支执行情况，并根据分支执行情况对代码进行编译优化；2. 是借助硬件、操作系统的能力，最为典型的是通过PMU获取程序执行情况，借助执行信息进行编译优化
3. LTO：通常也需要Profile信息，将Profile信息用于链接时优化，在链接时对函数、基本块进行优化，包括函数布局、基本块布局、函数拆分等。（如LLVM中的Call Graph Sort、HCS等都属于LTO优化）
4. PLO：在链接完成后，再次根据Profile信息等二进制文件进行优化（如LLVM中的BasicBlockSection、MFS等都属于PLO，此外BOLT也属于PLO）

LTO、PLO都是针对二进制文件的优化，一个是二进制文件生成前优化，一个是生成后优化，两者使用的优化方法类似

参考Facebook 2018年论文 `BOLT: A Practical Binary Optimizer for Data Centers and Beyond`中介绍了为什么PLO在叠加PGO、LTO后还有较好优化效果

BOLT项目针对二进制文件做了更多优化，例如针对代码布局实现了一系列算法——HFSort、C3、CodeSitechar、PH、PH.BB、TSP、ExtTSP等



## 11.5 扩展阅读：后缀树构造和应用

1995年Ukkonen实现了**对字符串从左到右遍历一次构造后缀树**的算法，而不是直接基于后缀子串进行后缀树构造的算法（对于大型字符串来说空间和时间不可估量）

1. 直观构造法：考虑显式节点和隐式节点，对于第二次出现的遍历字符，产生新的隐式节点。使用链表来跟踪插入位置，遍历一次字符串来构造后缀树。但每次都需要遍历尾部链表来添加新的字符串，而随字符越来越多，链表长度也随之增加，所以整体复杂度仍然是On方

2. Ukkonen构造法：考虑到“活跃点”、“计数器”的概念，和3个构造规则

   参考 `https://stackoverflow.com/questions/9452701/ukkonens-suffix-tree-algorithm-in-plain-english`

后缀树应用：

1. 查找字符串是否在另一个字符串中，是否为其子串
2. 求解字符串在另一个字符串的重复次数
3. 查找字符串的最长重复子串
4. 查找两个字符串的最长公共部分
