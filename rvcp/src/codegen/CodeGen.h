...
public:
  // Guards insertion point.
    ...
    template<class T>
    T *replace(Op *op, Value::Type resultTy, const std::vector<Value> &v, const std::vector<Attr*> &a) {
        setBeforeOp(op);
        auto opnew = create<T>(resultTy, v, a);
        op->replaceAllUsesWith(opnew);
        op->erase();
        return opnew;
    }
    ...
    template<class T>
    T *replace(Op *op, const std::vector<Value> &v, const std::vector<Attr*> &a) {
        setBeforeOp(op);
        auto opnew = create<T>(v, a);
        op->replaceAllUsesWith(opnew);
        op->erase();
        return opnew;
    }
